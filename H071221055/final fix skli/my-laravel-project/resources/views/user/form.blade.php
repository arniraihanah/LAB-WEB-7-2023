  ...
  <div class="w-full flex flex-col items-center">
    <form class=" w-full max-w-md space-y-4 flex flex-col" @if($editable) action="{{ route('user.update') }}" @else action="{{ route('user.create') }}" @endif method="POST">
      ...
      <div class="pt-4 flex flex-col w-full">
        <button type="submit" class="text-center px-3 py-2 rounded-md border border-emerald-700 bg-emerald-700 hover:bg-emerald-800 active:bg-emerald-900 text-white">@if($editable) Update @else Create @endif User</button>
      </div>
    </form>
  </div>
</main>
