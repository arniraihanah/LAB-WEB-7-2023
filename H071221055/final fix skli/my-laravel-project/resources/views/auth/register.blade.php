...
<header class="flex justify-between items-center px-6 py-3 w-full border-b border-b-slate-300">
  ...
  <div class="flex items-center">
    <a href="{{ route('auth.login_view') }}" class="min-w-[6rem] text-center px-3 py-2 rounded-md bg-emerald-700 hover:bg-emerald-800 active:bg-emerald-900 text-white">Login</a>
  </div>
</header>
...
<main class="flex flex-col items-center w-full py-6">
  <form class="mt-8 px-10 py-8 border rounded-lg border-slate-300 w-full max-w-md space-y-4" method="POST" action="{{ route('auth.register') }}">
    ...
    <div class="pt-4 pb-2 flex flex-col w-full">
      <button class="text-center px-3 py-2 rounded-md bg-emerald-700 hover:bg-emerald-800 active:bg-emerald-900 text-white w-full" type="submit">Register</button>
    </div>
  </form>
</main>
